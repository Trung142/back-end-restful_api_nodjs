...
const ShowEmployeer = async (req, res) => {
    try {
        const [row, files] = await poolmysql.execute('select * from Employees');
        ...
        const [data] = await poolmysql.execute('select * from Employees limit ? offset ?', [limit, offset])
        return res.status(200).json({
            message: 'ok',
            page: page,
            per_page: limit,
...
const CreateEmployeer = async (req, res) => {
    try {
        ...
        let sql = 'insert into Employees(first_name, last_name, phone_number, email, address, position, start_date, salary) values(?,?,?,?,?,?,?,?)';
        if (!last_name || !phone) {
            return res.status(200).json({
...
//delete
const deleteEmployeer = async (req, res) => {
    try {
        let userid = req.query.id;
        if (!userid) {
            return res.status(200).json({
        ...
        const [row, files] = await poolmysql.query(`delete from Employees where employee_id = ?`, [userid]);
        return res.status(200).json({
            message: 'ok',
            data: row,
            deleteAt: Date()
