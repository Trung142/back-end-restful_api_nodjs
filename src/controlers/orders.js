...
const ShowOrders = async (req, res) => {
    try {
        const [row, files] = await poolmysql.execute('select * from Orders');
        ...
        const [data] = await poolmysql.execute('select * from Orders limit ? offset ?', [limit, offset])
        return res.status(200).json({
            message: 'ok',
            page: page,
            per_page: limit,
...
const CreateOrders = async (req, res) => {
    try {
        ...
        let sql = 'insert into Orders(first_name, last_name, phone_number, email, address, position, start_date, salary) values(?,?,?,?,?,?,?,?)';
        if (!last_name || !phone) {
            return res.status(200).json({
...
const deleteOrders = async (req, res) => {
    try {
        ...
        const [row, files] = await poolmysql.query(`delete from Orders where order_id = ?`, [userid]);
        return res.status(200).json({
            message: 'ok',
            data: row,
            deleteAt: Date()
