...
const ShowPayment = async (req, res) => {
    try {
        const [row, files] = await poolmysql.execute('select * from Payments');
        ...
        const [data] = await poolmysql.execute('select * from Payments limit ? offset ?', [limit, offset])
        return res.status(200).json({
            message: 'ok',
            page: page,
            per_page: limit,
...
const CreatePayment = async (req, res) => {
    try {
        ...
        let sql = 'insert into Payments(first_name, last_name, phone_number, email, address, position, start_date, salary) values(?,?,?,?,?,?,?,?)';
        if (!last_name || !phone) {
            return res.status(200).json({
...
const deletePayment = async (req, res) => {
    try {
        ...
        const [row, files] = await poolmysql.query(`delete from Payments where payment_id = ?`, [userid]);
        return res.status(200).json({
            message: 'ok',
            data: row,
            deleteAt: Date()
